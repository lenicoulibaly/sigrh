...
public interface PrincipalAssRepo extends JpaRepository<PrincipalAss, Long>
{
    ...
    @Query("select new dgmp.sigrh.auth2.model.dtos.asignation.ReadPrincipalAssDTO(p.assId, p.intitule, p.ass.startsAt, p.ass.endsAt, p.ass.assStatus, case when p.ass.assStatus = 1 then 'Assignation courante' when p.ass.assStatus = 2 then 'Actif' when p.ass.assStatus = 3 then 'Inactif' else '' end, p.user.username, p.structure.strName, p.structure.strSigle, function('get_hierarchy_sigles', p.structure.strId) ) from PrincipalAss p where p.user.userId = ?1 and (locate(coalesce(?2, ''), upper(function('strip_accents', p.intitule)))>0 or locate(coalesce(?2, ''), upper(function('strip_accents', p.structure.strName)))>0 or locate(coalesce(?2, ''), upper(function('strip_accents', p.structure.strSigle)))>0 or (p.ass.assStatus = 1 and locate(coalesce(?2, ''), upper('Assignation courante'))>0)  or (p.ass.assStatus = 2 and locate(coalesce(?2, ''), upper('Actif'))>0)  or (p.ass.assStatus = 3 and locate(coalesce(?2, ''), upper('Inactif'))>0)) order by p.ass.assStatus asc")
