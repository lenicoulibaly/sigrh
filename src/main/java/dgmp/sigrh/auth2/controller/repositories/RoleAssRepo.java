...
public interface RoleAssRepo extends JpaRepository<RoleAss, Long>
{
    ...
    @Query("select (count(r.roleId)>0) from AppRole r where exists(select roleAss from RoleAss roleAss where roleAss.principalAss.assId = ?1 and roleAss.role.roleId = ?2 and roleAss.ass.assStatus = 1 and current_date between coalesce(roleAss.ass.startsAt, current_date ) and coalesce(roleAss.ass.endsAt, current_date))")
    boolean principalAssHasRole(Long principalAssId, Long roleId);

    @Query("select (count(r.roleId)>0) from AppRole r where exists(select roleAss from RoleAss roleAss where roleAss.principalAss.assId = ?1 and roleAss.role.roleName = ?2 and roleAss.ass.assStatus = 1 and current_date between coalesce(roleAss.ass.startsAt, current_date ) and coalesce(roleAss.ass.endsAt, current_date))")
    boolean principalAssHasRole(Long principalAssId, String roleName);

    @Query("select (count(r.roleId)>0) from AppRole r where exists(select roleAss from RoleAss roleAss where roleAss.principalAss.assId = ?1 and roleAss.role.roleId in ?2 and roleAss.ass.assStatus = 1 and current_date between coalesce(roleAss.ass.startsAt, current_date ) and coalesce(roleAss.ass.endsAt, current_date))")
    boolean principalAssHasAnyRole(Long principalAssId, List<Long> roleIds);

    @Query("select (count(r.roleId)>0) from AppRole r where exists(select roleAss from RoleAss roleAss where roleAss.principalAss.assId = ?1 and roleAss.role.roleName in ?2 and roleAss.ass.assStatus = 1 and current_date between coalesce(roleAss.ass.startsAt, current_date ) and coalesce(roleAss.ass.endsAt, current_date))")
    ...
    @Query("select roleAss.role.roleId from RoleAss roleAss where roleAss.principalAss.assId = ?1 and roleAss.ass.assStatus = 1 and (roleAss.ass.startsAt <> ?2 or roleAss.ass.endsAt <> ?3)")
    ...
    @Query("select r.role.roleId from RoleAss r where r.principalAss.assId = ?1 and r.ass.assStatus <> 1 and r.ass.startsAt = ?2 and r.ass.endsAt = ?3")
    Set<Long> findNoneActiveRoleIdsBelongingToPrincipalAss_sameDates(Long principalAssId, LocalDate startsAt, LocalDate endsAt);

    @Query("select r.role.roleId from RoleAss r where r.principalAss.assId = ?1 and r.ass.assStatus <> 1 and (r.ass.startsAt <> ?2 or r.ass.endsAt <> ?3)")
    ...
    @Query("select r from RoleAss r where r.principalAss.assId = ?1 and r.ass.assStatus = 1 and current_date between coalesce(r.ass.startsAt, current_date) and coalesce(r.ass.endsAt, current_date)")
    List<RoleAss> findActiveByPrincipalAss(Long assId);

    @Query("select r from RoleAss r where r.principalAss.assId = ?1 and locate(?2, function('strip_accents', r.role.roleName))>0 and r.ass.assStatus = 1 and current_date between coalesce(r.ass.startsAt, current_date) and coalesce(r.ass.endsAt, current_date)")
    Page<RoleAss> searchActiveByPrincipalAss(Long assId, String key, Pageable pageable);
}