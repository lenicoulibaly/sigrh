...
public interface StrRepo extends JpaRepository<Structure, Long>
{
    ...
    @Query("select s from Structure s where (locate(function('getStrCode', ?1), concat(s.strCode, '/')) = 1 or s.strId = ?1) and (coalesce(upper(function('strip_accents', s.strName) ), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('strip_accents',s.strSigle)), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('strip_accents',s.strTel)), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('strip_accents', s.strAddress)), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('strip_accents', s.situationGeo)), '') like upper(concat('%', coalesce(?2, ''), '%')) or upper(function('strip_accents',s.strType.name)) like upper(concat('%', coalesce(?2, ''), '%'))) and s.status = 'ACTIVE' order by s.strName")
    ...
    @Query("select s from Structure s where (locate(function('getStrCode', ?1), concat(s.strCode, '/')) = 1 or s.strId = ?1) and (coalesce(upper(function('strip_accents', s.strName) ), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('strip_accents',s.strSigle)), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('strip_accents',s.strTel)), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('strip_accents', s.strAddress)), '') like upper(concat('%', coalesce(?2, ''), '%')) or coalesce(upper(function('strip_accents', s.situationGeo)), '') like upper(concat('%', coalesce(?2, ''), '%')) or upper(function('strip_accents',s.strType.name)) like upper(concat('%', coalesce(?2, ''), '%'))) and s.status = ?3 order by s.strName")
    ...
    @Query("select s from Structure s where (coalesce(upper(s.strName), '') like upper(concat('%', coalesce(?1, ''), '%')) or upper(s.strSigle) like upper(concat('%', coalesce(?1, ''), '%')) or upper(s.strTel) like upper(concat('%', coalesce(?1, ''), '%')) or upper(s.strAddress) like upper(concat('%', coalesce(?1, ''), '%')) or upper(s.situationGeo) like upper(concat('%', coalesce(?1, ''), '%'))) and s.strType.typeId = ?2 and s.status = ?3 order by s.strName")
    ...
    @Query("select new dgmp.sigrh.structuremodule.model.dtos.str.ChangeAnchorDTO(s.strId, s.strType.typeId, s.strParent.strId, s.strName, s.strSigle, s.strLevel) from Structure s where s.strId = ?1")
