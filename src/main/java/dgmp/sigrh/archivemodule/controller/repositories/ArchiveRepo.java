...
public interface ArchiveRepo extends JpaRepository<Archive, Long>
{
    ...
    @Query("select a from Archive a where a.status = 'ACTIVE' and a.agent.agentId = ?1 and (a.archiveType.typeId in ?3 or locate(?2, coalesce(upper(function('strip_accents', a.description)), '')) >0  or locate(?2, coalesce(upper(function('strip_accents', a.archiveType.name)), '')) >0 )")
